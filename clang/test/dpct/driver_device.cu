...
void test() {
  ...
  // CHECK-NEXT: */
  // CHECK: result3 = dpct::get_device(device).get_mem_base_addr_align_in_bytes();
  ...
  // CHECK-NEXT: */
  // CHECK: result4 = dpct::get_device(device).get_max_register_size_per_work_group();
  cuDeviceGetAttribute(&result4,CU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK, device);
  std::cout << " result4 " << result4 << std::endl;

  // CHECK: result5 = dpct::get_device(device).has(sycl::aspect::usm_host_allocations);
...
int main(){
  ...
  // CHECK: result1 = dpct::get_device(device).get_max_work_group_size();
  ...
  // CHECK: result1 = dpct::get_major_version(dpct::get_device(device));
  // CHECK: result2 = dpct::get_minor_version(dpct::get_device(device));
  cuDeviceComputeCapability(&result1, &result2, device);

  // CHECK: MY_SAFE_CALL([&](){
  // CHECK-NEXT:   result1 = dpct::get_major_version(dpct::get_device(device));
  // CHECK-NEXT:   result2 = dpct::get_minor_version(dpct::get_device(device));
  // CHECK-NEXT:   return 0;
  // CHECK-NEXT: }());
  ...
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(result1 = dpct::device_count()));
  MY_SAFE_CALL(cuDeviceGetCount(&result1));

  // CHECK: result1 = dpct::device_count();
  ...
  // CHECK: memcpy(name, dpct::get_device(device).get_info<sycl::info::device::name>().c_str(), 90);
  cuDeviceGetName(name, 90, device);

  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(memcpy(name, dpct::get_device(device).get_info<sycl::info::device::name>().c_str(), 90)));
  MY_SAFE_CALL(cuDeviceGetName(name, 90, device));
  // CHECK: size = dpct::get_device(device).get_device_info().get_global_mem_size();
